...
#     return render(request, 'createadmin.html',{'tot_cust':tot_cust,'invoice':invoice})
...
# @api_view(['POST'])
# def customer_and_invoice_view(request):
#     if request.method == 'POST':
...
#             'customerid': request.data.get('custid')
#         }

#         # Extract invoice data from the request
#         invoice_data = {
#             'customer': request.data.get('custname'),
#             'date': request.data.get('date'),
#             'amount': request.data.get('Amount'),
#             'status': request.data.get('status'),
#             'invoice_id': request.data.get('invid')
#         }

#         # Serialize customer and invoice data
#         customer_serializer = CustomerSerializer(data=customer_data)
#         invoice_serializer = InvoiceSerializer(data=invoice_data)

#         # Validate and save customer and invoice data
#         if customer_serializer.is_valid() and invoice_serializer.is_valid():
#             customer = customer_serializer.save()
#             invoice = invoice_serializer.save(customer=customer)
#             return Response({'message': 'Customer and invoice created successfully', 'customer_id': customer.id, 'invoice_id': invoice.id}, status=status.HTTP_201_CREATED)
#         else:
#             return Response({'errors': {'customer': customer_serializer.errors, 'invoice': invoice_serializer.errors}}, status=status.HTTP_400_BAD_REQUEST)



# @api_view(['GET'])
# def get_customer_invoice_data(request):
#     customers = Customer.objects.all()
#     invoices = Invoice.objects.all()

#     return render(request, 'customerlist.html', {'customers': customers, 'invoices': invoices})
